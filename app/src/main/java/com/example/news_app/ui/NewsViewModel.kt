...
class NewsViewModel (val app:Application,val newsRepository: NewsRepository): AndroidViewModel(app) {
    ...
    fun searchNews(searchQuery: String,shouldPaginate: Boolean){
        viewModelScope.launch {
            safeSearchNewsCall(searchQuery,shouldPaginate)
        }
    }
    ...
    private fun handleSearchNewsResponse(response:Response<NewsResponse>,shouldPaginate:Boolean): Resource<NewsResponse>{
        if(response.isSuccessful){
            response.body()?.let { resultResponse->
            if(shouldPaginate) {
                searchNewsPage++
    ...
    private suspend fun safeSearchNewsCall(searchQuery: String,shouldPaginate:Boolean){
        searchNews. postValue(Resource.Loading())
        try{
            if(hasInternetConnect()){

                val response = newsRepository.searchNews(searchQuery,searchNewsPage)
                searchNews.postValue(handleSearchNewsResponse(response,shouldPaginate))
            }else{
